...
const AjoutAffectation = () => {
  const { id_materiel: idMaterielParam } = useParams();
  ...
  // Charger employés, matériels et services au chargement
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Si on a un idMaterielParam dans l'URL, on le pré-remplit et met à jour le service précédent
        if (idMaterielParam) {
          const materielChoisi = matRes.data.find(m => String(m.id_materiel) === idMaterielParam);
          if (materielChoisi) {
            setForm(form => ({
              ...form,
              id_materiel: String(materielChoisi.id_materiel),
              id_service_precedent: materielChoisi.id_service ? String(materielChoisi.id_service) : '',
            }));
          }
        }
  ...
  // Gestion du changement de matériel (mise à jour automatique du service précédent)
  const handleMaterielChange = (e) => {
    ...
    setForm(form => ({
      ...form,
      id_materiel: idMat,
      id_service_precedent: materielChoisi && materielChoisi.id_service ? String(materielChoisi.id_service) : '',
      id_service_nouveau: '',
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.id_service_precedent === form.id_service_nouveau) {
      setError('Le service précédent et le service nouveau doivent être différents.');
      setMessage(null);
      return;
    }

    try {
      await axios.post('http://localhost:3000/api/affectations', form);
      setMessage('Affectation ajoutée avec succès !');
      setError(null);
      setForm({
        id_employe: '',
        id_materiel: '',
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-2xl mb-4 font-bold">Ajouter une Affectation</h2>
